...
| **SEO AutoPilot** | `SEO_AUTOPILOT_README.md`, `docs/seo-autopilot/`, `@seo-autopilot/*` packages | **Engine/platform product** under active Engineering Bible delivery — **no customer UI yet** |
...
| SEO AutoPilot web | `apps/web` — **scaffold-only**, no React/Next dependency, no UI | Confirmed by code |
...
**None.** `apps/web` and `packages/ui` explicitly forbid UI decisions until a UX Bible / approved sprint.
