...
**Sprint D6 delivered and LOCKED:** Opportunity Engine on `/opportunities`.  
**Sprint D7 delivered:** Guided Action Plans on `/opportunities/:slug/plan` (session-only manual completion). No auth, Google connections, or SEO scores. `packages/ui` remains scaffold-only.
