...
**Sprint D1 delivered** in `apps/web`: calm AppShell, Home (`/`), Learning Centre (`/learn`), FAQ articles (`/learn/:slug`). No auth, Ask, Google connections, or fabricated business metrics. `packages/ui` remains scaffold-only.
