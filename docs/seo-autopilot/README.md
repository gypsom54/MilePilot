# SEO AutoPilot — Developer Documentation

Architecture foundation documentation for Sprint 0, plus locked Engineering Bible volumes.

| Document | Topic |
| --- | --- |
| [NORTH_STAR.md](./NORTH_STAR.md) | Mission / philosophy |
| [SPRINT_0.md](./SPRINT_0.md) | Sprint objective, AC, tests, rollback |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Monorepo topology + principles |
| [VOLUME_3_ASK_SEO_AUTOPILOT.md](./VOLUME_3_ASK_SEO_AUTOPILOT.md) | **Engineering Bible Vol. 3 — Ask orchestration (spec only)** |
| [ASK_SEO_AUTOPILOT.md](./ASK_SEO_AUTOPILOT.md) | Ask summary + implementation lock |
| [VOLUME_4_BUSINESS_DISCOVERY.md](./VOLUME_4_BUSINESS_DISCOVERY.md) | **Engineering Bible Vol. 4 — Business Discovery** |
| [SPRINT_1.md](./SPRINT_1.md) | Sprint 1 Business Discovery implementation plan |
| [VOLUME_4_BUSINESS_DISCOVERY_READINESS.md](./VOLUME_4_BUSINESS_DISCOVERY_READINESS.md) | Volume 4 pointer / Sprint 1 entry |
| [VOLUME_5_MARKET_INTELLIGENCE.md](./VOLUME_5_MARKET_INTELLIGENCE.md) | **Engineering Bible Vol. 5 — Market Intelligence** |
| [SPRINT_2.md](./SPRINT_2.md) | Sprint 2 Market Intelligence implementation plan |
| [VOLUME_6_WEBSITE_INTELLIGENCE.md](./VOLUME_6_WEBSITE_INTELLIGENCE.md) | **Engineering Bible Vol. 6 — Website Intelligence** |
| [SPRINT_3.md](./SPRINT_3.md) | Sprint 3 Website Intelligence implementation plan |
| [CREATING_AN_ENGINE.md](./CREATING_AN_ENGINE.md) | How to create an Intelligence Engine |
| [EVENTS.md](./EVENTS.md) | Event Bus communication |
| [KNOWLEDGE_GRAPH.md](./KNOWLEDGE_GRAPH.md) | Knowledge Graph SDK |
| [TESTING.md](./TESTING.md) | Engine testing framework |

## Build & test

```bash
pnpm install
pnpm run build:seo-autopilot
pnpm run test:seo-autopilot
```
