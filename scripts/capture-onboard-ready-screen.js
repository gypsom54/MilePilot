...
async function main() {
  ...
  await page.evaluate(() => {
    ...
    localStorage.setItem('mp_onboard_step', 'ready');
    localStorage.setItem('mp_tracking_mode', 'autopilot');
    localStorage.setItem('mp_location_choice', 'granted');
    if (typeof initOnboardReady === 'function') initOnboardReady();
    if (typeof showScreen === 'function') showScreen('onboardReady');
  });
  await page.waitForSelector('#onboardReady.active', { timeout: 15000 });
  await page.waitForTimeout(1400);
  const file = path.join(OUT, 'onboarding-ready-screen5.png');
