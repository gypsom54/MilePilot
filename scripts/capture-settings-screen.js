...
async function main() {
  ...
  await page.evaluate(() => {
    ...
    localStorage.setItem('mp_notifications_choice', 'granted');
    localStorage.setItem('mp_email', 'jonathan@example.com');
    if (typeof showSettings === 'function') showSettings();
  });
  await page.waitForSelector('#settings.active', { timeout: 15000 });
  await page.waitForTimeout(500);
  const file = path.join(OUT, 'settings-ui-lock-v8.43.24.png');
