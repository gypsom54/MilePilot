...
async function main() {
  ...
  await page.evaluate(() => {
    localStorage.clear();
    localStorage.setItem('mp_onboard_step', 'enableSetup');
    if (typeof initEnableSetup === 'function') initEnableSetup();
    if (typeof showScreen === 'function') showScreen('permissions');
  });
  await page.waitForSelector('#permissions.active', { timeout: 15000 });
  await page.waitForTimeout(500);
  const file = path.join(OUT, 'onboarding-permissions-screen4.png');
