...
/** Future: QR opens interactive route viewer (timeline + map replay). Not built yet — verify URL for now. */
export function buildInteractiveReportUrl(reportId) {
  return `${APP_URL}/?report=${encodeURIComponent(reportId)}&viewer=1`;
}

export function buildReportEmailPreviewText(report) {
  const period = report.period || "Daily";
  const map = {
    Daily: "Today's mileage summary is ready.",
    Weekly: "This week's mileage summary is ready.",
    Monthly: "This month's mileage summary is ready.",
    Annual: "Your annual mileage summary is ready.",
  };
  return map[period] || "Your mileage summary is ready.";
}
...
function periodGlanceTitle(period, shifts) {
  if (period === "Daily") return "Today's Summary";
  if (period === "Weekly") return "This Week at a Glance";
  if (period === "Monthly") {
    const d = shifts.length ? new Date(shifts[0].startISO) : new Date();
    return `${d.toLocaleDateString("en-GB", { month: "long", year: "numeric" })} at a Glance`;
  }
  return `${period} Summary`;
}

function periodInsightLine(a) {
  const { period, shifts, busiest, longestJ } = a;
  if (!shifts.length) return null;
  if ((period === "Monthly" || period === "Weekly") && busiest?.miles > 0) {
    const when = period === "Monthly" ? "this month" : "this week";
    return `Your busiest day ${when} was ${busiest.day} with ${fmtMi(busiest.miles)} business miles.`;
  }
  if (period === "Daily" && longestJ) {
    return `Your longest trip today was ${fmtMi(longestJ.miles)} miles (${fmtDurationShort(longestJ.seconds)}).`;
  }
  return null;
}
...
function drawFooter(doc, a, margin, contentW) {
  ...
  drawThinRule(doc, margin, footY - 88, contentW, "#DDE6F2");

  doc.fillColor(BRAND.muted).font("Helvetica-Bold").fontSize(7).text("Generated by MilePilot", margin, footY - 78, { width: contentW, align: "center" });
  doc.fillColor(BRAND.blue).font("Helvetica-Bold").fontSize(7.5).text(BRAND_TAGLINE, margin, footY - 66, { width: contentW, align: "center", characterSpacing: 0.6 });

  doc.fillColor(BRAND.muted).font("Helvetica").fontSize(6.5).text(
  ...
  doc.fillColor(BRAND.muted).font("Helvetica").fontSize(6.5).text(
    `Background Tracking: Enabled  ·  Business Trips Included: ${a.totals.journeys}  ·  Pending Trips Excluded: ${a.pendingCount || 0}`,
    margin,
    footY - 40,
    { width: contentW, align: "center" }
  );
  doc.fillColor(BRAND.muted).font("Helvetica").fontSize(6.5).text(
    "This report has been automatically generated by MilePilot.",
    margin,
    footY - 28,
    { width: contentW, align: "center" }
  );
  ...
  const name = firstName(a.driver);
  if (name) {
    doc.fillColor(BRAND.muted).font("Helvetica").fontSize(6).text(`Prepared for ${name}`, margin, footY - 16, { width: contentW, align: "center" });
  }
  doc.fillColor(BRAND.text).font("Helvetica-Bold").fontSize(6).text(`Report ${a.reportId}`, margin, footY - (name ? 6 : 16), { width: contentW, align: "center" });
  doc.fillColor(BRAND.muted).font("Helvetica").fontSize(6).text(`Page ${pdfPageNum}`, margin, footY + 2, { width: contentW, align: "right" });
  doc.fillColor(BRAND.blue).font("Helvetica-Bold").fontSize(6.5).text("app.milepilot.uk", margin, footY + 2, { width: contentW, align: "left", link: APP_URL });
}
...
function drawAtAGlanceSummary(doc, a, margin, contentW, y) {
  const title = periodGlanceTitle(a.period, a.shifts);
  const boxH = a.pendingCount > 0 ? 132 : 118;
  doc.roundedRect(margin, y, contentW, boxH, 6).fill(PDF.cardFill).strokeColor(PDF.cardStroke).lineWidth(0.6).stroke();

  let ly = y + PDF.unit * 2;
  doc.fillColor(BRAND.navy).font("Helvetica-Bold").fontSize(11).text(title, margin + PDF.unit * 2, ly, { width: contentW - PDF.unit * 4 });
  ly += PDF.unit * 2.5;

  const colW = (contentW - PDF.unit * 4) / 2;
  const rows = [
    ["Business Miles", `${fmtMi(a.totals.mi)} mi`],
    ["Driving Time", fmtShiftTime(a.totals.sec)],
    ["HMRC Estimate", money(a.totals.hmrc)],
    ["Business Trips", String(a.totals.journeys)],
    ["Avg Trip Length", a.totals.journeys ? `${fmtMi(a.avgMilesShift)} mi` : "—"],
    ["Longest Trip", a.longestJ ? `${fmtMi(a.longestJ.miles)} mi` : "—"],
  ];
  if (a.pendingCount > 0) {
    rows.push(["Pending Trips (Excluded)", String(a.pendingCount)]);
  }

  rows.forEach((row, i) => {
    const col = i % 2;
    const rowIdx = Math.floor(i / 2);
    const rx = margin + PDF.unit * 2 + col * colW;
    const ry = ly + rowIdx * 22;
    doc.fillColor(BRAND.muted).font("Helvetica").fontSize(7).text(row[0], rx, ry, { width: colW - 8 });
    doc.fillColor(BRAND.text).font("Helvetica-Bold").fontSize(9).text(row[1], rx, ry + 9, { width: colW - 8 });
  });

  ly += Math.ceil(rows.length / 2) * 22 + PDF.unit;
  const insight = periodInsightLine(a);
  if (insight) {
    doc.fillColor(BRAND.blue).font("Helvetica").fontSize(8.5).text(insight, margin + PDF.unit * 2, ly, { width: contentW - PDF.unit * 4, lineGap: 1.2 });
  }
...
function drawDashboardPage(doc, a, margin, contentW, pageW, pageH) {
  ...
  y = drawAtAGlanceSummary(doc, a, margin, contentW, y);
...
function drawJourneyTimelineCard(doc, s, margin, contentW, y, tripIndex) {
  const cardH = 108;
  const thumbW = 88;
  const thumbH = 52;
  const cx = margin + PDF.unit * 2.5;
  const textW = contentW - thumbW - PDF.unit * 5;
  const tripLabel = tripIndex != null ? `BUSINESS TRIP ${tripIndex + 1}` : "BUSINESS TRIP";
  ...
  iy += 14;
  doc.fillColor(BRAND.navy).font("Helvetica-Bold").fontSize(7).text(tripLabel, cx, iy, { characterSpacing: 0.8 });
  iy += 10;
...
function drawJourneyTimeline(doc, a, margin, contentW, y, pageW) {
  if (!a.shifts.length) {
    ...
    doc.fillColor(BRAND.muted).font("Helvetica").fontSize(9.5).text("No business trips in this period.", margin + PDF.unit * 2, y + 22, { width: contentW - 32, align: "center" });
    return y + 56 + PDF.sectionGap;
  }

  a.shifts.forEach((s, idx) => {
    if (y + 115 > footerTop(doc.page.height) - PDF.footerH) {
      drawFooter(doc, a, margin, contentW);
      doc.addPage({ margin: 0 });
      drawPageBackground(doc, pageW, doc.page.height);
      drawPageWatermark(doc, pageW, doc.page.height);
    ...
    y = drawJourneyTimelineCard(doc, s, margin, contentW, y, idx);
  });
...
export async function buildPdfBuffer(report) {
  ...
  return new Promise((resolve, reject) => {
    ...
    y = drawSectionHeading(doc, a.period === "Daily" ? "Daily Journey Timeline" : "Business Trips", margin, y, contentW);
...
export function buildReportEmailHtml(report) {
  const a = analyseReport(report);
  const name = firstName(a.driver) || "there";
  const headline = periodReportTitle(a.period);
  const preview = buildReportEmailPreviewText(report);
  ...
  const attachmentBox = `<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;border:1px solid rgba(110,180,255,.28);border-radius:16px;background:rgba(13,107,255,.1);">
    <tr><td style="padding:22px 20px;text-align:center;">
      ...
      <p style="margin:0;font-size:14px;color:#B9C8DD;line-height:1.55;">Tap the attachment in your mail app to open, save, or share — suitable for HMRC and accountant records.</p>
    </td></tr>
  </table>`;

  const btnStyle = "display:inline-block;width:100%;max-width:320px;font-size:16px;font-weight:600;text-decoration:none;padding:16px 24px;border-radius:14px;letter-spacing:-0.01em;box-sizing:border-box;";
...
<tr><td style="padding:0 20px;">
  ...
  <p style="margin:0 0 8px;font-size:16px;color:#EAF2FF;line-height:1.5;">Hello ${name},</p>
  <p style="margin:0 0 28px;font-size:15px;color:#B9C8DD;line-height:1.55;">Your ${a.period === "Daily" ? "daily" : a.period === "Weekly" ? "weekly" : a.period.toLowerCase()} business mileage summary is ready. Business trips only — pending and personal journeys are excluded.</p>
  ${pendingNotice}
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
    ...
    ${metric("Business Trips", String(a.totals.journeys))}
    ${metric("Pending Trips", a.pendingCount > 0 ? String(a.pendingCount) + " (excluded)" : "0")}
  </table>
  ${attachmentBox}
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:12px;">
    <tr><td align="center" style="padding-bottom:12px;">
      <a href="${appUrl}" style="${btnStyle}background:linear-gradient(180deg,#1E88FF,#0D6BFF);color:#FFFFFF;">View Report</a>
    </td></tr>
    <tr><td align="center">
      <a href="${downloadUrl}" style="${btnStyle}background:rgba(255,255,255,.08);color:#EAF2FF;border:1px solid rgba(110,180,255,.28);">Download PDF</a>
    </td></tr>
  </table>
  <p style="margin:0 0 36px;font-size:14px;color:#93A8C4;line-height:1.6;text-align:center;">Generated by MilePilot · ${BRAND_TAGLINE_HTML}</p>
...
export function buildReportSubject(report) {
  const period = report.period || "Daily";
  const map = {
    Daily: "Your MilePilot Daily Report",
    Weekly: "Your MilePilot Weekly Report",
    Monthly: "Your MilePilot Monthly Report",
    Annual: "Your MilePilot Annual Report",
  };
  return map[period] || "Your MilePilot Report";
}
