...
export function OnboardingAnalysis({
  completedSteps,
  businessName,
}: OnboardingAnalysisProps) {
  return (
    <div className="text-center sm:text-left">
      <p className="text-sm font-medium tracking-wide text-[#8b95a5]">
        Aura goes to work
      </p>
      <h1 className="mt-6 text-3xl font-semibold tracking-tight text-[#080f1a] sm:text-4xl">
        Setting up {businessName.trim() || "your business"}
      </h1>
        ...
        {ANALYSIS_STEPS.map((step, index) => {
          ...
          return (
            <li
              key={step.id}
              className={cn(
                "flex items-center gap-4 transition-all duration-500 motion-reduce:transition-none",
                !isComplete &&
                  !isActive &&
